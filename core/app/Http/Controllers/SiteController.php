...
class SiteController extends Controller
{
    ...
    public function blogDetails($id, $slug)
    {
        ...
        $recentBlogs = Frontend::where('data_keys', 'blog.element')->where('id', '!=', $blog->id)->orderBy('id', 'desc')->limit(5)->get();
        $pageTitle = strLimit($blog->data_values->title,30);
        ...
        $seoContents['image']              = getImage('assets/images/frontend/blog/' . @$blog->data_values->blog_image, '770x570');
        $seoContents['image_size']         = '770x570';
    ...
    public function subscribe(Request $request)
    {
        ...
        $ifExist = Subscriber::where('email', $request->email)->first();
        if (!$ifExist) {
            Subscriber::create([
                'email' => $request->email
            ]);
